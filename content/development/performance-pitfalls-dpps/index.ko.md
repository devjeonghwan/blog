...
바로 삼각형 교차 검사 내에서 사용되는 **Dot Product(내적)** 에  `_mm_dp_ps(DPPS)`, `_mm256_dp_ps(VDPPS)`를 적용해도 생각보다 전체 성능이 증가하지 않는 것이었습니다.
...
제가 회사에서 주로 개발하고 있는 **N차원 행렬 연산 프레임워크** 도 그렇고, AI 워크로드에서도 Dot Product는 벡터가 연속적으로 존재하는 배치 연산 상황을 가정하기 때문에, `FMA` 후 `Scalar Reduce`를 하는 패턴을 자주 이용합니다.
