...
스칼라 C 코드 기준으로는 `17.14` 에서 `13.97` 로 개선되었습니다.
...
바로 삼각형 교차 검사 내에서 사용되는 **내적(Dot Product)** 에  `_mm_dp_ps(DPPS)`, `_mm256_dp_ps(VDPPS)`를 적용해도 생각보다 전체 성능이 증가하지 않는 것이었습니다.

* ***DPPS:** Dot Product of Packed Single Precision Floating-Point Values

이 녀석들은 명령어 이름 자체에 **"Dot Product"** 가 있듯이 하나의 벡터에 대한 내적 연산을 하드웨어 수준에서 구현하고 있습니다.
...
물론 스칼라 형태로만 작성해도 요즘의 컴파일러는 잘 최적화하겠지만, 그래도 내적에 해당하는 명령어를 사용한 측이 더 빠를 것 이라는 기대를 하고 테스트를 해보았습니다.
...
왜냐하면 수평 덧셈 연산인 `HADDPS` 대신, **CPU 포트** 제약이 상대적으로 적은 `SHUFPS`(재배치)와 `ADDPS`(일반 덧셈)로 조합해 구현하게 되면, 수평 연산 병목(낮은 처리량, 스케줄링 자유도 부족)을 줄이고, 전체 루프 관점에서 더 높은 처리량을 얻을 가능성이 커집니다.

> 실제로 `HADDPS`의 Latency는 7, CPI는 2이며, `SHUFPS`와 `ADDPS`는 각각 Latency는 1, 4, CPI는 0.5, 0.5입니다.

* ***Latency:** Latency: 명령 실행부터 결과가 나올 때까지 걸리는 시간 (Cycle)
* ***CPI:** 명령어 1개를 처리하는 데 필요한 평균 시간(Cycle) (예, CPI: 0.5 = 1클럭에 2개 처리)
...
정확한 내부 구현은 공개되어 있지 않기 때문에, 인텔 엔지니어가 아니고서야 딱 어떻다고 말하기는 어렵습니다. 하지만 예상해 보자면

1. `DPPS`는 `imm8`을 통해서 어떤 Lane을 연산하고(상위 4비트) 어떤 Lane에 기록할지(하위 4비트)를 결정할 수 있는데 이것이 하드웨어 비용을 발생시킨다.
2. `DPPS`가 내부적으로 수평 연산을 포함하고, 특정 포트 의존성이 있어 명령어 수준 병렬성(Instruction-Level Parallelism, ILP)을 확보하기 어렵다. (`A안, HADDPS` vs `B안, ADDPS + SHUFPS` 문제와 본질적으로 유사)
...
제가 회사에서 주로 개발하고 있는 **N차원 행렬 연산 프레임워크** 도 그렇고, AI 워크로드에서도 내적은 벡터가 연속적으로 존재하는 배치 연산 상황을 가정하기 때문에, `FMA` 후 `Scalar Reduce`를 하는 패턴을 자주 이용합니다.
