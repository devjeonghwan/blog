...
사실 직관적으로는 `DIVSS`가 당연히 `RCPSS`보다 느려야 한다고 생각이 들었습니다. 왜냐하면 `RCPSS`는 **역수(Reciprocal)** 을 구할때 비싼 나눗셈 연산을 사용하지 않도록 구현된 하드웨어 수준의 명령어니까요.
...
따라서, 병목이 해소되면서 **파이프라인 효율(Microarchitecture Usage)** 이 **6.2%에서 25.6%으로 4배** 이상 개선된 것입니다.
