...
이 예제는 절대적인 최고 성능을 목표로 만든 구현이라기보다, `CUDA Stream`이 **커널 실행**, **메모리 복사**, **CPU 측 후처리**와 **화면 갱신**의 레이턴시를 어떻게 가릴 수 있는지 설명하기 위한 교육용 데모에 가깝습니다.

따라서 시뮬레이션 처리와 화면 렌더링을 의도적으로 1:1로 맞추고 있으며, GPU를 최대한 오래 바쁘게 유지하는 구조보다는 Sync와 Stream의 차이가 드러나는 구조를 우선했습니다.
